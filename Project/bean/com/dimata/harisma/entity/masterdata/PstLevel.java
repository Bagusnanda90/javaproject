...
public class PstLevel extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {    
    ...
    public static final int FLD_MAX_LEVEL_APPROVAL = 10;
    public static final int FLD_HR_APPROVAL = 11;
    ...
    public static Level fetchExc(long oid) throws DBException {        
        try {            
            ...
            level.setMaxLevelApproval(pstLevel.getLong(FLD_MAX_LEVEL_APPROVAL));
            level.setHr_approval(pstLevel.getInt(FLD_HR_APPROVAL));
    ...
    public static long insertExc(Level level) throws DBException {        
        try {            
            ...
            pstLevel.setLong(FLD_MAX_LEVEL_APPROVAL, level.getMaxLevelApproval());
            pstLevel.setInt(FLD_HR_APPROVAL, level.getHr_approval());
            pstLevel.insert();            
    ...
    public static void resultToObject(ResultSet rs, Level level) {
        try {
            ...
            level.setMaxLevelApproval(rs.getLong(PstLevel.fieldNames[PstLevel.FLD_MAX_LEVEL_APPROVAL]));
            level.setHr_approval(rs.getInt(PstLevel.fieldNames[PstLevel.FLD_HR_APPROVAL]));
    ...
    public static Level getLevelByPoint(int point) {
        ...
        try {
            String sql = "SELECT * FROM " + TBL_HR_LEVEL + " WHERE "
                    + PstLevel.fieldNames[PstLevel.FLD_LEVEL_POINT] + " = '" + point + "'";
    ...
    public static Level getLevelByMaxLevelPoint(String empId) {
        
        DBResultSet dbrs = null;
        Level level = new Level();
        try {
            String sql = "SELECT * FROM `hr_level` hl  INNER JOIN `hr_employee` he  ON he.`LEVEL_ID` = hl.`LEVEL_ID` WHERE `EMPLOYEE_ID` IN ("+empId+") ORDER BY hl.`LEVEL_POINT`";
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                resultToObject(rs, level);
     ...
     public static Vector getAllLevelByPoint(int point) {
        DBResultSet dbrs = null;
