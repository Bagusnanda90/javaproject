...
public class PstFamilyMember extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector listDataTable(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            ...
            String sql = "SELECT FM.*,FR." + PstFamRelation.fieldNames[PstFamRelation.FLD_FAMILY_RELATION_TYPE] + " FROM " + TBL_HR_FAMILY_MEMBER + " AS FM "
                    ...
                    + " LEFT JOIN " + PstEducation.TBL_HR_EDUCATION + " AS EDU "
                    + "ON FM." + PstFamilyMember.fieldNames[PstFamilyMember.FLD_EDUCATION_ID] + " = "
                    + "EDU." + PstEducation.fieldNames[PstEducation.FLD_EDUCATION_ID]
                    + " LEFT JOIN " + PstReligion.TBL_HR_RELIGION + " AS RL "
                    + "ON FM." + PstFamilyMember.fieldNames[PstFamilyMember.FLD_RELIGION_ID] + " = "
                    + "RL." + PstReligion.fieldNames[PstReligion.FLD_RELIGION_ID];
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
    ...
    public static int getCountDataTable(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT( FM." + PstFamilyMember.fieldNames[PstFamilyMember.FLD_FAMILY_MEMBER_ID] + ") FROM " + TBL_HR_FAMILY_MEMBER + " AS FM "
                    + " INNER JOIN " + PstFamRelation.TBL_HR_FAM_RELATION + " AS FR "
                    + "ON FM." + PstFamilyMember.fieldNames[PstFamilyMember.FLD_RELATIONSHIP] + " = "
                    + "FR." + PstFamRelation.fieldNames[PstFamRelation.FLD_FAMILY_RELATION_ID]
                    + " INNER JOIN " + PstEducation.TBL_HR_EDUCATION + " AS EDU "
                    + "ON FM." + PstFamilyMember.fieldNames[PstFamilyMember.FLD_EDUCATION_ID] + " = "
                    + "EDU." + PstEducation.fieldNames[PstEducation.FLD_EDUCATION_ID]
                    + " INNER JOIN " + PstReligion.TBL_HR_RELIGION + " AS RL "
                    + "ON FM." + PstFamilyMember.fieldNames[PstFamilyMember.FLD_RELIGION_ID] + " = "
                    + "RL." + PstReligion.fieldNames[PstReligion.FLD_RELIGION_ID];
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
