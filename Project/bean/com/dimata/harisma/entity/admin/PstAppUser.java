...
public class PstAppUser extends DBHandler implements I_DBInterface, I_DBType, I_Persintent  {
    ...
    /**
     * Description : User Admin, mempunyai hakakses lebih
     * Date : 2016-05-10
     * Author : Hendra Putu
     */
    public static final int FLD_ADMIN_STATUS = 14;
    ...
    //value for Excel Input/Output
    public static final int ADMIN_OFF = 0;
    public static final int ADMIN_ON = 1;

    public static final String[] adminKey = {"OFF","ON"};
    public static final int[] adminValue = {0,1};
    ...
    public static  final String[] fieldNames = {
        ...
        ,"EMPLOYEE_ID", "POSITION_LEVEL_ID","EXCEL_IO", "ADMIN_STATUS"
    } ;
    
    public static int[] fieldTypes = {
        TYPE_PK + TYPE_LONG + TYPE_ID, TYPE_STRING,   TYPE_STRING,  TYPE_STRING,  TYPE_STRING,  TYPE_STRING,
        TYPE_DATE, TYPE_DATE, TYPE_INT, TYPE_DATE, TYPE_STRING, TYPE_LONG, TYPE_STRING, TYPE_INT, TYPE_INT
    };
    ...
    public static AppUser fetch(long oid) {
        ...
        try {
            ...
            entObj.setAdminStatus(pstObj.getInt(FLD_ADMIN_STATUS));
        }
        catch(DBException e) {
    ...
    public static long insert(AppUser entObj) throws DBException  {
        ...
        pstObj.setInt(FLD_ADMIN_STATUS, entObj.getAdminStatus());
...
//            System.out.println(sql);
    ...
    public static int updatePassword(long oid, String password) {
        DBResultSet dbrs = null;
        int status = 0;
        try {
            String sql = "";
            if (password != null && password.length() > 0) {
                sql = "UPDATE " + TBL_APP_USER
                    + " SET " + fieldNames[FLD_PASSWORD]
                    + " = \"" + password + "\""
                    + " WHERE " + fieldNames[FLD_USER_ID]
                    + " = " + oid;
            }
            ...
            System.err.println("\tupdate User Password error : " + e.toString());
        } finally {
            DBResultSet.close(dbrs);
        }
        return status;
