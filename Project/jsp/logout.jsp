<%@page import="com.dimata.harisma.entity.attendance.MachineTransaction"%>
<%@page import="com.dimata.harisma.entity.attendance.PstMachineTransaction"%>
...
{
    int cekOut=0;
        cekOut = FRMQueryString.requestInt(request, "cekOut");
    if (cekOut == 1){
        Employee employee = new Employee();
        try{
            employee = PstEmployee.fetchExc(emplx.getOID());
        } catch (Exception e){
        ...
        machineTransaction.setDateTransaction(todayD);
        machineTransaction.setPosted(0);
        machineTransaction.setMode("0");
        machineTransaction.setStation("System");
        machineTransaction.setNote("transaction from System");
        long n = PstMachineTransaction.insertExc(machineTransaction);
    }
	if(userSession.isLoggedIn()==true)
	{
		...
		session.removeValue(SessUserSession.HTTP_SESSION_NAME);
                session.removeValue("machineLogin");
...
window.setTimeout(function() {
    window.location.href = '<%=approot%>/login.jsp';
}, 5000);
