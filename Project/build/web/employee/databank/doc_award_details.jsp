...
<%@page import="com.dimata.harisma.entity.employee.PstEmpAward"%>
<%@page import="com.dimata.harisma.entity.employee.EmpAward"%>
...
long awardId = FRMQueryString.requestLong(request, "awardId");
...
try {
    award = PstEmpAward.fetchExc(awardId);
} catch (Exception exc) {
...
if (award.getDocId() > 0 || oidEmpDoc > 0) {
    try {
        empDoc = PstEmpDoc.fetchExc(award.getDocId());
    } catch (Exception exc){}
    docNumber = empDoc.getDoc_number();
} else {
        ...
        for (int i = 0; i < docNumArray.length; i++) {
            ...
            if (element.equals("NUMBER")){
                ...
                if (listEmpDoc != null && listEmpDoc.size() > 0){
                    ...
                    docNumber = docNumber.replaceAll("NUMBER", "1");
                }
...
empDocAward.setDoc_number(docNumber);
empDocAward.setDate_of_issue(award.getAwardDate());
empDocAward.setDetails(empDoc.getDetails());
...
if (award.getDocId() == 0) {
    ...
    empDocField.setEmp_doc_id(oidDoc);
    empDocField.setClassName("ALLFIELD");

    long oidEmpDocTitle = PstEmpDocField.insertExc(empDocField);
    
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    String awardDate = sdf.format(award.getAwardDate());
    empDocField.setObject_name("AWARDDATE");
    empDocField.setObject_type(0);
    empDocField.setValue(awardDate);
    empDocField.setEmp_doc_id(oidDoc);
    empDocField.setClassName("ALLFIELD");
    
    long oidEmpDocDate = PstEmpDocField.insertExc(empDocField);
...
function cmdSave(){
	...
	document.frmEmpDoc.action="doc_award_details.jsp";
                                          ...
                                          <td height="8"  colspan="3"> 
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0">                                              
                                             ...
                                             <% 
                                                ...
                                                String where2 = " " + PstEmpDocRecipient.fieldNames[PstEmpDocRecipient.FLD_EMP_DOC_ID] + " = \"" + oidEmpDoc + "\"";
                                             ...
                                             <tr align="left" valign="top" > 
                                              ...
                                              <tr > 
                                                <td >
                                                    <% if (fromCareerPath != 1){%>
                                                     <td>
                                                         <a href="javascript:cmdSave('<%=oidEmpDoc%>')" class="btn-sm btn-primary command" type="button">Save</a> 
<!--                                                         || <a href="javascript:cmdUpload('<%=oidEmpDoc%>')" class="command">Upload</a> -->
<!--                                                         || <a href="javascript:cmdOpen('<%=empDoc1.getFileName()%>')" class="command">Download</a>  -->
                                                     </td>
                                                    <% } %>
                                                </td>
                                              </tr>   
