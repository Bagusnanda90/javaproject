...
//update by satrya 2012-10-16
    ...
    public static void updateStockManagementNormal(LeaveAlClosing leaveAlClosing) {
        ...
        while (next == true && iterasi < 50) {
            ...
            String entitle_ll_1 = getDateIntervalMonth(startIntrvlLL_1, employee.getCommencingDate());
            String entitle_ll_2 = getDateIntervalMonth(startIntrvlLL_2, employee.getCommencingDate());
    ...
    public static Vector listLLClosingLLByParameter(SrcLeaveAppAlClosing srcLeaveAppAlClosing) {
        ...
        try {
            ...
            String sql = "SELECT LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STOCK_ID] + ",LLM."
                    ...
                    " WHERE ( LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STATUS] + (srcLeaveAppAlClosing.getStatus()==2?
    ...
    public static Date getExpiredDate(long stock_id, int expired_ll) {
        ...
        try {
            ...
            String sql = "SELECT " + PstLlStockExpired.fieldNames[PstLlStockExpired.FLD_EXPIRED_DATE]
                    ...
                    + " AND " + PstLlStockExpired.FLD_EXPIRED_LAST + " = " + PstLlStockExpired.LAST_TRUE
...
//update by satrya 2012-11-08
    ...
    /**
     * @Author  Priska 
     * @Desc    Untuk memproses closing AL yang menggunakan commencing dan periode 
     * @param   VleaveAlClosing by commencing periode
     */
    public static void ProcessClosingCommencingPeriod2(Vector VleaveAlClosing, I_Leave leaveConfig){
        ...
        if (VleaveAlClosing != null && VleaveAlClosing.size() > 0) {
            ...
            for (int i = 0; i < VleaveAlClosing.size(); i++){
                ...
                if (countData == 0){ // jika stock aktif dan tidak aktif belum ada
                  ...
                  float tmpentitle = leaveConfig.getALEntitleAnualLeaveAlaCommecingPeriode(leaveAlClosing);
                   // float tmpentitle = leaveConfig.getALEntitleAnualLeave(level.getLevel(), empCategory.getEmpCategory(), LoS, employee.getCommencingDate(),new Date());
                    ...
                    Date tmpEntitleDt = getTmpDateEntitleCommecingPeriode(leaveAlClosing);
                    ...
                    Date tmpDtClosing = getTmpDateEntitleCommecingPeriode(leaveAlClosing);
                    //cara mencari tmpDtCLosingnya berbeda untuk commencingperiode
...
//                    double difInMonths = (dateEntile.getTime() - entDtClosing.getTime())/2592000000d;
//                    if (difInMonths > 12 ){
//                        difInMonths = 12;
//                    }
//                    int nn = (int) difInMonths;
//                    tmpentitle  = (float) nn;
//                     int LoSFromCom = (int) DateCalc.dayDifference(employee.getCommencingDate(),dateEntile);
//                    if (LoSFromCom < 365){
//                        tmpentitle = 0;
//                    }
                    
                    
                    //tmpentitle = Float.parseFloat(""+(new Double(df2.format(difInMonths)).doubleValue())); 
                     //update by satrya 2013-11-22
                    float entitleAlExtra = 0;//update by satrya 2014-06-10 karena al Extra sdh di tambahkan di service manager leaveConfig.getAlExtraAl(level.getLevel(), empCategory.getEmpCategory(), LoS, employee.getCommencingDate(), new Date());
                    Date dtEntitleAlExtra =  null;//update by satrya 2014-06-10 karena al Extra sdh di tambahkan di service manager leaveConfig.getAlExtraDateAl(entitleAlExtra, employee.getCommencingDate());
    ...
    //agus priska 20160112
     public static  Date getTmpDateEntitleCommecingPeriode(LeaveAlClosing leaveAlClosing) {
        Date DateiEntitleAL = null;
        try {
            ...
            if (entDtClosing.getTime() == entDtCommencing.getTime()){ //jika pertama closing maka kesini
                DateiEntitleAL = (Date) entDtCommencingPlusOneYears.clone();
            } else {  //jika closing kedua dan seterusnya
                entDtClosing.setYear(entDtClosing.getYear()+1);
                entDtClosing.setMonth(0);
                entDtClosing.setDate(1);
                DateiEntitleAL = (Date) entDtClosing.clone();
            }   
           
            DateiEntitleAL = new Date(DateiEntitleAL.getTime()-(24*60*60*1000));
          
        } catch (Exception exc) {
            
            System.out.println(exc);
        }
        return DateiEntitleAL;
    }
