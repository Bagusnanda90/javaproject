...
public class PstTraining extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static String getResponList(int limitStart, int recordToGet, String whereClause, String order) {
        String str = "";
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_HR_TRAINING;
            ...
            while (rs.next()) {
                ...
                str += "<tr>";
                str += "<td>"+no+"</td>";
                str += "<td>"+rs.getString(PstTraining.fieldNames[PstTraining.FLD_NAME])+"</td>";
                str += "<td>"+rs.getLong(PstTraining.fieldNames[PstTraining.FLD_TYPE])+"</td>";
                str += "</tr>";
                no++;
            }
            rs.close();
