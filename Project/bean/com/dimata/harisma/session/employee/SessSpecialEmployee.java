...
+",SUBSEC."+PstSubSection.fieldNames[PstSubSection.FLD_SUB_SECTION]                    
...
+",EMP."+PstEmployee.fieldNames[PstEmployee.FLD_INDENT_CARD_VALID_TO]
+",EMP."+PstEmployee.fieldNames[PstEmployee.FLD_SECONDARY_ID_NO]
+",EMP."+PstEmployee.fieldNames[PstEmployee.FLD_SECONDARY_ID_VALID];        
...
+ " LEFT JOIN  " + PstSubSection.TBL_SUB_SECTION + " AS SUBSEC  ON (EMP." + PstEmployee.fieldNames[PstEmployee.FLD_SUB_SECTION_ID] + " = SUBSEC." + PstSubSection.fieldNames[PstSubSection.FLD_SUB_SECTION_ID]+")" 
...
//            }
          ...
          if(searchSpecialQuery.getArrDepartmentId(0)!=null){
            ...
            String[] subSectionId = searchSpecialQuery.getArrSubSectionId(0);
            //String[] sectionId = (String[]) vparam.get(3);
            if (subSectionId!=null && !( (subSectionId[0].equals("0")))) {
                whereClause += " AND (";
                for (int i=0; i<subSectionId.length; i++) {
                    whereClause = whereClause + " EMP."+PstEmployee.fieldNames[PstEmployee.FLD_SUB_SECTION_ID]+
                                " = "+ subSectionId[i] + " OR ";
                    if (i==(subSectionId.length-1)) {
                        whereClause = whereClause.substring(0, whereClause.length()-4);
                    }
                }
                whereClause += " )";
...
//                Race race = new Race();
                ...
                sessTmpSpecialEmployee.setPostalCodeEmployee(rs.getInt(PstEmployee.fieldNames[PstEmployee.FLD_POSTAL_CODE]));
                sessTmpSpecialEmployee.setPhoneEmg(rs.getString(PstEmployee.fieldNames[PstEmployee.FLD_PHONE_EMG]));
                ...
                sessTmpSpecialEmployee.setSubSection(rs.getString(PstSubSection.fieldNames[PstSubSection.FLD_SUB_SECTION]));
		...
		sessTmpSpecialEmployee.setTaxMarital(rs.getString("MARTAX."+PstMarital.fieldNames[PstMarital.FLD_MARITAL_STATUS]));
                ...
                /* Add by Hendra Putu | 2015-12-15 */
                sessTmpSpecialEmployee.setNoRekening(rs.getString(PstEmployee.fieldNames[PstEmployee.FLD_NO_REKENING]));
                sessTmpSpecialEmployee.setAddressEmg(rs.getString((PstEmployee.fieldNames[PstEmployee.FLD_ADDRESS_EMG])));
                sessTmpSpecialEmployee.setIdCardValid(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_INDENT_CARD_VALID_TO]));
                sessTmpSpecialEmployee.setSecondaryIdNo(rs.getString(PstEmployee.fieldNames[PstEmployee.FLD_SECONDARY_ID_NO]));
                sessTmpSpecialEmployee.setSecondaryIdValid(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_SECONDARY_ID_VALID]));
