<%-- 
    Document   : familymember_view
    Created on : Dec 28, 2015, 10:48:11 AM
    Author     : Dimata 007
--%>
...
<html>
    <head>
        ...
        <title>Databank - <%=dictionaryD.getWord(I_Dictionary.FAMILY_MEMBER) %></title>
    ...
    <body>
        ...
        <div id="menu_utama">
            <span id="menu_title">Databank <strong style="color:#333;"> / </strong> <%=dictionaryD.getWord(I_Dictionary.FAMILY_MEMBER) %></span>
        </div>
        <% if (oidEmployee != 0) {%>
            <div class="navbar">
                <ul style="margin-left: 97px">
                    ...
                    <li class="active"> <a href="familymember_view.jsp" class="tablink"><%=dictionaryD.getWord(I_Dictionary.FAMILY_MEMBER) %></a> </li>
        ...
        <div class="content-main">
            <div class="box">
                <div id="box_title">Daftar <%=dictionaryD.getWord(I_Dictionary.FAMILY_MEMBER) %></div>
                <div id="box_content">
                    <table class="tblStyle">
                        <tr>
                            <td class="title_tbl">No</td>
                            <td class="title_tbl">Nama Lengkap</td>
                            <td class="title_tbl">Jenis Kelamin</td>
                            <td class="title_tbl">Hubungan</td>
                            <td class="title_tbl">Tanggal Lahir</td>
                            <td class="title_tbl">Pekerjaan</td>
                            <td class="title_tbl">Alamat</td>
                            <td class="title_tbl">Pendidikan</td>
                            <td class="title_tbl">Agama</td>
                        </tr>
                    <%
                    String whereClause = PstFamilyMember.fieldNames[PstFamilyMember.FLD_EMPLOYEE_ID] + "=" + oidEmployee;
                    String orderClause = PstFamilyMember.fieldNames[PstFamilyMember.FLD_RELATIONSHIP] + "," + PstFamilyMember.fieldNames[PstFamilyMember.FLD_BIRTH_DATE] + " ASC";
                    Vector listFamilyMem = PstFamilyMember.list(0, 0, whereClause, orderClause);
                    if (listFamilyMem != null && listFamilyMem.size()>0){
                        for(int i=0; i<listFamilyMem.size(); i++){
                            FamilyMember famMember = (FamilyMember)listFamilyMem.get(i);;
                        %>
                        <tr>
                            <td><%=(i+1)%></td>
                            <td><%=famMember.getFullName()%></td>
                            <td><%=PstEmployee.sexKey[famMember.getSex()]%></td>
                            <td><%= sessEmpView.getRelationship(Long.valueOf(famMember.getRelationship())) %></td>
                            <td><%=famMember.getBirthDate()%></td>
                            <td><%=famMember.getJob()%></td>
                            <td><%=famMember.getAddress()%></td>
                            <td><%= sessEmpView.getEducationName(famMember.getEducationId()) %></td>
                            <td><%= sessEmpView.getReligion(famMember.getReligionId())%></td>
                        </tr>
                        <%
                        }
                    }
