...
public class PstTrainingHistory extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
  ...
  public static Vector listDataTable(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT EMP.* FROM "+PstTrainingHistory.TBL_HR_TRAINING_HISTORY+""
                    + " AS EMP LEFT JOIN "+PstTraining.TBL_HR_TRAINING+""
                    + " AS TRA ON EMP."+PstTrainingHistory.fieldNames[PstTrainingHistory.FLD_TRAINING_ID]+" ="
                    + " TRA."+PstTraining.fieldNames[PstTraining.FLD_TRAINING_ID]+" "
                    + "LEFT JOIN "+PstEmpDoc.TBL_HR_EMP_DOC+" AS DOC "
                    + "ON EMP."+PstTrainingHistory.fieldNames[PstTrainingHistory.FLD_EMP_DOC_ID]+" = "
  ...
  public static int getCountDataTable(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(EMP."+PstTrainingHistory.fieldNames[PstTrainingHistory.FLD_TRAINING_HISTORY_ID]+")"
    ...
    public static Hashtable fetchExcHashtable(long oid) throws DBException {
        try {
            Hashtable empTraining = new Hashtable();
            TrainingHistory trainingHistory = new TrainingHistory();
            PstTrainingHistory pstTrainingHistory = new PstTrainingHistory(oid);
            Employee employee = new Employee();
            
            trainingHistory.setEmployeeId(pstTrainingHistory.getLong(FLD_EMPLOYEE_ID));
            try { employee = PstEmployee.fetchExc(trainingHistory.getEmployeeId()); } catch (Exception exc){}
            empTraining.put(PstTrainingHistory.fieldNames[PstTrainingHistory.FLD_EMPLOYEE_ID], employee.getFullName());
            
            trainingHistory.setTrainingTitle(pstTrainingHistory.getString(FLD_TRAINING_TITLE));
            empTraining.put(PstTrainingHistory.fieldNames[PstTrainingHistory.FLD_TRAINING_TITLE], trainingHistory.getTrainingTitle());
            
            trainingHistory.setStartDate(pstTrainingHistory.getDate(FLD_START_DATE));
            SimpleDateFormat sdf = new SimpleDateFormat("dd MMMM yyyy");
            String startDate = sdf.format(trainingHistory.getStartDate());

            empTraining.put(PstTrainingHistory.fieldNames[PstTrainingHistory.FLD_START_DATE], startDate);
            
            trainingHistory.setEndDate(pstTrainingHistory.getDate(FLD_END_DATE));
            SimpleDateFormat sdf1 = new SimpleDateFormat("dd MMMM yyyy");
            String endDate = sdf1.format(trainingHistory.getEndDate());

            empTraining.put(PstTrainingHistory.fieldNames[PstTrainingHistory.FLD_END_DATE], endDate);
