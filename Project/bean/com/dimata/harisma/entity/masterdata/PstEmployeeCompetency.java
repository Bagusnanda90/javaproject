...
public class PstEmployeeCompetency extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector listDataTable(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT EMP.* FROM " +PstEmployeeCompetency.TBL_EMPLOYEE_COMPETENCY+ " AS EMP"
                    + " INNER JOIN " +PstCompetency.TBL_COMPETENCY+ " AS COMP "
                    + "ON EMP."+PstEmployeeCompetency.fieldNames[PstEmployeeCompetency.FLD_COMPETENCY_ID]+" = "
                    + "COMP."+PstCompetency.fieldNames[PstCompetency.FLD_COMPETENCY_ID]+" ";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
}
            if (order != null && order.length() > 0) {
...
public static int getCountDataTable(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql ="SELECT COUNT(EMP."+PstEmployeeCompetency.fieldNames[PstEmployeeCompetency.FLD_EMPLOYEE_COMP_ID]+") FROM " +PstEmployeeCompetency.TBL_EMPLOYEE_COMPETENCY+ " AS EMP"
                    + " INNER JOIN " +PstCompetency.TBL_COMPETENCY+ " AS COMP "
                    + "ON emp."+PstEmployeeCompetency.fieldNames[PstEmployeeCompetency.FLD_COMPETENCY_ID]+" = "
                    + "comp."+PstCompetency.fieldNames[PstCompetency.FLD_COMPETENCY_ID]+" ";
            if (whereClause != null && whereClause.length() > 0) {
