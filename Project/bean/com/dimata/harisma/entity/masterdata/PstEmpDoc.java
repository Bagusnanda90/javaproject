...
public class PstEmpDoc extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
 ...
 public static final int FLD_DOC_STATUS = 19;
 public static final int FLD_NOTE = 20;
 public static final int FLD_NOTE_STATUS = 21;
   ...
   public PstEmpDoc() {
    ...
    public static EmpDoc fetchExc(long oid) throws DBException {
        try {
            ...
            empDoc.setNote(pstEmpDoc.getString(FLD_NOTE));
            empDoc.setNote_status(pstEmpDoc.getInt(FLD_NOTE_STATUS));
    ...
    public static Hashtable fetchExcHashtable(long oid) throws DBException {
        try {
            ...
            empDoc.setRequest_date(pstEmpDoc.getDate(FLD_REQUEST_DATE));
            empDocH.put(PstEmpDoc.fieldNames[PstEmpDoc.FLD_REQUEST_DATE], (empDoc.getRequest_date() != null?Formater.formatDate(empDoc.getRequest_date(),"dd MMMM yyyy"):"") );
           
            empDoc.setDoc_number(pstEmpDoc.getString(FLD_DOC_NUMBER));
            empDocH.put(PstEmpDoc.fieldNames[PstEmpDoc.FLD_DOC_NUMBER], (empDoc.getDoc_number() != null ? empDoc.getDoc_number() : "" ));
            
            empDoc.setDate_of_issue(pstEmpDoc.getDate(FLD_DATE_OF_ISSUE));
            empDocH.put(PstEmpDoc.fieldNames[PstEmpDoc.FLD_DATE_OF_ISSUE], (empDoc.getDate_of_issue() != null? Formater.formatDate(empDoc.getDate_of_issue(),"dd MMMM yyyy"):"") );
    ...
    public static int listSpecialCount(int limitStart, int recordToGet, String whereClause, String order, searchEmpDoc seaDoc) {
      ...
      public static void resultToObject(ResultSet rs, EmpDoc empDoc) {
        try {
            ...
            empDoc.setDoc_status(rs.getInt(PstEmpDoc.fieldNames[PstEmpDoc.FLD_DOC_STATUS]));
            empDoc.setNote(rs.getString(PstEmpDoc.fieldNames[PstEmpDoc.FLD_NOTE]));
            empDoc.setNote_status(rs.getInt(PstEmpDoc.fieldNames[PstEmpDoc.FLD_NOTE_STATUS]));
...
/**
    ...
    public static int updateNote(long empDocId, String note, int noteStatus){
        int result = 0;
        DBResultSet dbrs = null;
            try {
                String sql = "UPDATE " + PstEmpDoc.TBL_HR_EMP_DOC
                        + " SET " +PstEmpDoc.fieldNames[PstEmpDoc.FLD_NOTE]+ " = '" + note
                        + "' , "+ PstEmpDoc.fieldNames[PstEmpDoc.FLD_NOTE_STATUS] +" = "+noteStatus+" WHERE " + PstEmpDoc.fieldNames[PstEmpDoc.FLD_EMP_DOC_ID]
                        + " = " + empDocId;
                result = DBHandler.execUpdate(sql);
            } catch (Exception e) {
                System.out.println("Exc update emp document status : " + e.toString());
