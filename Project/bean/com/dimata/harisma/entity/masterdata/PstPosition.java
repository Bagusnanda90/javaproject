...
//        "--None--"
    ...
    public static Vector listMappingPos(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT pos.* FROM " + TBL_HR_POSITION +" AS pos "
                        + " LEFT JOIN " + PstPositionCompany.TBL_POSITION_COMPANY + " AS com "
                        + " ON pos."+PstPosition.fieldNames[PstPosition.FLD_POSITION_ID]+" = com."+PstPositionCompany.fieldNames[PstPositionCompany.FLD_POSITION_ID]
                        + " LEFT JOIN " + PstPositionDivision.TBL_POSITION_DIVISION + " AS dv "
                        + " ON pos."+PstPosition.fieldNames[PstPosition.FLD_POSITION_ID]+" = dv."+PstPositionDivision.fieldNames[PstPositionDivision.FLD_POSITION_ID]
                        + " LEFT JOIN " + PstPositionDepartment.TBL_POSITION_DEPARTMENT + " AS dept "
                        + " ON pos."+PstPosition.fieldNames[PstPosition.FLD_POSITION_ID]+" = dept."+PstPositionDepartment.fieldNames[PstPositionDepartment.FLD_POSITION_ID]
                        + " LEFT JOIN " + PstPositionSection.TBL_POSITION_SECTION + " AS sec "
                        + " ON pos."+PstPosition.fieldNames[PstPosition.FLD_POSITION_ID]+" = sec."+PstPositionSection.fieldNames[PstPositionSection.FLD_POSITION_ID]
                        + " LEFT JOIN " + PstPositionSubSection.TBL_POSITION_SUBSECTION + " AS subsec "
                        + " ON pos."+PstPosition.fieldNames[PstPosition.FLD_POSITION_ID]+" = subsec."+PstPositionSubSection.fieldNames[PstPositionSubSection.FLD_POSITION_ID];
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
