...
public class PstEmpEducation extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector listDataTable(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT EMP.* FROM "+PstEmpEducation.TBL_HR_EMP_EDUCATION+""
                    + " AS EMP INNER JOIN "+PstEducation.TBL_HR_EDUCATION+""
                    + " AS EDU ON EMP."+PstEmpEducation.fieldNames[PstEmpEducation.FLD_EDUCATION_ID]+" ="
                    + " EDU."+PstEducation.fieldNames[PstEducation.FLD_EDUCATION_ID]+"";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
}
    ...
    public static int getCountDataTable(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(EMP."+PstEmpEducation.fieldNames[PstEmpLanguage.FLD_EMP_LANGUAGE_ID]+") "
                    + "FROM "+PstEmpEducation.TBL_HR_EMP_EDUCATION+""
                    + " AS EMP INNER JOIN "+PstEducation.TBL_HR_EDUCATION+""
                    + " AS EDU ON EMP."+PstEmpEducation.fieldNames[PstEmpEducation.FLD_EDUCATION_ID]+" ="
                    + " EDU."+PstEducation.fieldNames[PstEducation.FLD_EDUCATION_ID]+"";
            if (whereClause != null && whereClause.length() > 0) {
