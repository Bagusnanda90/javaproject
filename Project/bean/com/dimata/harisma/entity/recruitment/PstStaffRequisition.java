...
public class PstStaffRequisition extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
	...
	public static final  String TBL_HR_STAFF_REQUISITION = "hr_staff_requisition";//"HR_STAFF_REQUISITION";
	...
	public static Vector list(int limitStart,int recordToGet, String whereClause, String order){
		...
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT req.* FROM " + TBL_HR_STAFF_REQUISITION+" AS req "
                                + "INNER JOIN "+PstDepartment.TBL_HR_DEPARTMENT+" AS dep "
                                + "ON req."+PstStaffRequisition.fieldNames[PstStaffRequisition.FLD_DEPARTMENT_ID]+" = "
                                + "dep."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT_ID]+" "
                                + "INNER JOIN "+PstSection.TBL_HR_SECTION+" AS sec "
                                + "ON req."+PstStaffRequisition.fieldNames[PstStaffRequisition.FLD_SECTION_ID]+" = "
                                + "sec."+PstSection.fieldNames[PstSection.FLD_SECTION_ID]+" "
                                + "INNER JOIN "+PstPosition.TBL_HR_POSITION+" AS pos "
                                + "ON req."+PstStaffRequisition.fieldNames[PstStaffRequisition.FLD_POSITION_ID]+" = "
                                + "pos."+PstPosition.fieldNames[PstPosition.FLD_POSITION_ID]+" "
                                + "INNER JOIN "+PstEmpCategory.TBL_HR_EMP_CATEGORY+" AS cat "
                                + "ON req."+PstStaffRequisition.fieldNames[PstStaffRequisition.FLD_EMP_CATEGORY_ID]+" = "
	...
	public static int getCount(String whereClause){
		...
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT COUNT(req."+ PstStaffRequisition.fieldNames[PstStaffRequisition.FLD_STAFF_REQUISITION_ID] + ") FROM " + TBL_HR_STAFF_REQUISITION+" AS req "
                                + "INNER JOIN "+PstDepartment.TBL_HR_DEPARTMENT+" AS dep "
                                + "ON req."+PstStaffRequisition.fieldNames[PstStaffRequisition.FLD_DEPARTMENT_ID]+" = "
                                + "dep."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT_ID]+" "
                                + "INNER JOIN "+PstSection.TBL_HR_SECTION+" AS sec "
