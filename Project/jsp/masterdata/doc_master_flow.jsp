<%-- 
    Document   : doc_master_flow
    Created on : 31-Dec-2015, 12:09:10
    Author     : PRISKA
--%>
...
<%!
...

	{
		...
		ctrlist.setHeaderStyle("listgentitle");
                ctrlist.addHeader("Edit","8%");
		ctrlist.addHeader("Flow Title","10%");
		ctrlist.addHeader("Flow Index","2%");
		...
		int index = -1;
                    ...
                    Hashtable listpositionH = new Hashtable(); 
                    ...
                    for (int s = 0; s< listposition.size(); s++) {
                        ...
                        listpositionH.put(String.valueOf(position.getOID()), position.getPosition());
                    }
                    ...
                    Hashtable listlevelH = new Hashtable();
                    for (int s = 0; s< listlevel.size(); s++) {
                        ...
                        listlevelH.put(String.valueOf(level.getOID()), level.getLevel()); 
                    }
		...
		for (int i = 0; i < objectClass.size(); i++) {
			 ...
			 if(index == i && (iCommand == Command.EDIT || iCommand == Command.ASK)){
				...
				rowx.add(ControlCombo.draw(frmObject.fieldNames[FrmDocMasterFlow.FRM_FIELD_FLOW_INDEX], "formElemen", null, String.valueOf(docMasterFlow.getFlow_index()), frmObject.getindexValue(), frmObject.getindexKey(), ""));
                    ...
                    Vector comp_key = new Vector(1, 1);
                    Vector listComp = PstCompany.list(0, 0, "", " COMPANY ");
                    Hashtable listCompH= new Hashtable(); 
                    comp_value.add(""+0);
                    comp_key.add("select");
                    for (int c = 0; c < listComp.size(); c++) {
                        Company div = (Company) listComp.get(c);
                        comp_key.add(div.getCompany());
                        comp_value.add(String.valueOf(div.getOID()));
                        listCompH.put(String.valueOf(div.getOID()), div.getCompany());
                    }
                  ...
                  //division
                    ...
                    //update priska 2015-01-29
                       div_key.add("-select-");
                        ...
                        strWhere = PstDivision.fieldNames[PstDivision.FLD_COMPANY_ID] + "=" +  "" + docMasterFlow.getCompany_id() ;//oidCompany; 
                        Vector listDiv = PstDivision.list(0, 0, strWhere, " DIVISION ");
                        Hashtable listDivH= new Hashtable(); 
                        
                        for (int c = 0; c < listDiv.size(); c++) {
                        Division div = (Division) listDiv.get(c);
                        div_key.add(div.getDivision());
                        div_value.add(String.valueOf(div.getOID()));
                        listDivH.put(String.valueOf(div.getOID()), div.getDivision());
                    }  
                    ...
                    strWhereDept = PstDepartment.TBL_HR_DEPARTMENT+"."+PstDepartment.fieldNames[PstDepartment.FLD_DIVISION_ID] + "=" + "" + docMasterFlow.getDivision_id(); //oidDivision;
                    Vector listDept = PstDepartment.list(0, 0, strWhereDept, " DEPARTMENT ");
                    Hashtable listDeptH= new Hashtable(); 
                    ...
                    dept_key.add(dept.getDepartment());
                    dept_value.add(String.valueOf(dept.getOID()));
                    listDeptH.put(String.valueOf(dept.getOID()), dept.getDepartment());
                    } 
                   
                                rowx.add("<button id=\"btn\" onclick=\"cmdEdit('"+String.valueOf(docMasterFlow.getOID())+"')\">Edit</button>");
                                rowx.add(""+docMasterFlow.getFlow_title()+" <input type=\"hidden\" name=\""+frmObject.fieldNames[FrmDocMasterFlow.FRM_FIELD_DOC_MASTER_ID] +"\" value=\""+objEntity.getDoc_master_id()+"\" class=\"elemenForm\"> ");
                                rowx.add(String.valueOf(docMasterFlow.getFlow_index()));
                                
                                rowx.add(""+listCompH.get(String.valueOf(docMasterFlow.getCompany_id())));
                                rowx.add(""+listDivH.get(String.valueOf(docMasterFlow.getDivision_id())));
                                rowx.add(""+listDeptH.get(String.valueOf(docMasterFlow.getDepartment_id())));
                                rowx.add(""+listlevelH.get(String.valueOf(docMasterFlow.getLevel_id())));
                                rowx.add(""+listpositionH.get(String.valueOf(docMasterFlow.getPosition_id())));
...
try {docMaster = PstDocMaster.fetchExc(oidDocMaster); }catch (Exception e){}
...
function cmdUpdateCDDS(n){
                ...
                document.frmDocMasterFlow.action="doc_master_flow.jsp?n="+n+"";
...
function cmdAdd(masterId){
	...
	document.frmDocMasterFlow.action="doc_master_flow.jsp?DocMasterId="+masterId+"";
	document.frmDocMasterFlow.submit();
}
...
function cmdAsk(oidDocMasterFlow){
	...
	document.frmDocMasterFlow.prev_command.value="<%=prevCommand%>";
	document.frmDocMasterFlow.action="doc_master_flow.jsp";
	document.frmDocMasterFlow.submit();
}
...
function cmdBack(masteroid){
	...
	document.frmDocMasterFlow.action="doc_master_flow.jsp?DocMasterId="+masteroid+"";
	document.frmDocMasterFlow.submit();
}
          ...
          <td height="20">
		    <font color="#FF6600" face="Arial"><strong>
			  <!-- #BeginEditable "contenttitle" -->Masterdata 
                          &gt; DocMasterFlow &gt; <%=docMaster.getDoc_title() %><!-- #EndEditable --> 
            </strong></font>
	      </td>
        </tr>
        <tr> 
