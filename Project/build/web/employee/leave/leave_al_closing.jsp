...
public String drawList(JspWriter outObj,Vector objectClass,Vector result,I_Leave leaveConfig)
{
	ControlList ctrlist = new ControlList();
        ...
        String SetLeaveExtend = "12";
            try {
                SetLeaveExtend = String.valueOf(PstSystemProperty.getValueByName("SET_DEFAULT_LEAVE_EXTEND"));//menambahkan system properties
            } catch (Exception e) {
	...
	ctrlist.reset();
		...
		LeaveAlClosingList leaveAlClosingList = new LeaveAlClosingList();
                ...
                leaveAlClosingList = (LeaveAlClosingList)objectClass.get(i);
                       //entitle nya salah sebelumnya karena salah closing time jadi harus dikembalikan ke commencing date
                if (leaveAlClosingList.getAlStockManagementId() != 0 && (leaveConfig.getALEntitleBy() == leaveConfig.AL_ENTITLE_BY_COMMENCING_PERIODE) ){
                    Date comm = (Date)  leaveAlClosingList.getCommancingDate().clone();
                        Date entitleDateX = (Date) leaveAlClosingList.getEntitledDate().clone();
                        int LoS = (int) DateCalc.dayDifference(comm, entitleDateX );
                        Date newda = new Date();
                        //comm.setYear(comm.getYear()+1);
                        if (LoS < 365){
                            try{
                ...
                try { employee = PstEmployee.fetchExc(leaveAlClosingList.getEmpId()); }catch (Exception e){}
                ...
                String drawlist = ControlCombo.draw("Extended"+i, null, ""+SetLeaveExtend, vectValExtend, vectListExtend,"");	
...
//end sementara                                                                                                          
                ...
                if(statusApplicationNotClose==true || !okNextEntitle){
                    ...
                    if(countData > 1){
                        ...
                        if(resultStatus == 4){
                            ...
                            rowx.add("<center><input type=\"checkbox\" name=\"Close"+i+"\" value=\"1\"></center>");} else {rowx.add("");rowx.add("");}
                        ...
                        if(leaveConfig.getALEntitleBy() == leaveConfig.AL_ENTITLE_ANUAL||leaveConfig.getALEntitleBy()==I_Leave.AL_ENTITLE_BY_PERIOD){ /* entitle by periode */
                            ...
                            if( leaveAlClosingList.getEntitledDate().getTime() >= selectedDateCom.getTime()){
                              ...
                              /*  Calendar startDateR = Calendar.getInstance();
                                ...
                                */
                            rowx.add("<input type=\"text\" name=\"data_extend"+i+"\" value=\""+residu+"\" size=\"5\">"+" "+drawlist);
                            if (empCategory.getEntitleLeave() == PstEmpCategory.ENTITLE_YES){rowx.add("<center><input type=\"checkbox\" name=\"Close"+i+"\" value=\"1\"></center>");} else {rowx.add("");}
                            rowx.add("");   
                                                           } else {   
        ...
        if(result != null && result.size() > 0){
            ...
            for(int idx = 0 ; idx < result.size() ; idx++){
                ...
                Employee employee = new Employee();
                EmpCategory empCategory = new EmpCategory();
                try { employee = PstEmployee.fetchExc(leaveAlNoStockClosingList.getEmpId()); }catch (Exception e){}
                try { empCategory = PstEmpCategory.fetchExc(employee.getEmpCategoryId()); }catch (Exception e){}
...
if(iCommand==Command.ACTIVATE){
    ...
    if(resultClose.size()>0){        
        ...
        if(leaveConfig.getALEntitleBy() == leaveConfig.AL_ENTITLE_ANUAL||leaveConfig.getALEntitleBy()== I_Leave.AL_ENTITLE_BY_PERIOD){ /* menggunakan closing by period */
            ...
            SessLeaveClosing.ProcessClosingCommencingPeriod2 (resultClose,leaveConfig);
