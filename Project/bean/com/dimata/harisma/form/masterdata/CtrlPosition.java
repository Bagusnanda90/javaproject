...
public class CtrlPosition extends Control implements I_Language 
{
	...
	public int getStart() { return start; }
        ...
        public int action(int cmd , long oidPosition, I_Atendance attdConfig){
            return action(cmd, oidPosition, attdConfig, "");
        }

	public int action(int cmd , long oidPosition, I_Atendance attdConfig, String oidDeleteAll){
		msgString = "";
				...
				break;
                                ...
                                for (String asset : splits) {
                                    if (asset != "") {
                                        ...
                                        if (oidDoc != 0) {
                                            try {
                                                long oid = PstPosition.deleteExc(oidDoc);
                                                if (oid != 0) {
                                                    msgString = FRMMessage.getMessage(FRMMessage.MSG_DELETED);
                                                    excCode = RSLT_OK;
                                                } else {
