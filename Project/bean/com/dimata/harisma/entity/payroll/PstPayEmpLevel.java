...
public class PstPayEmpLevel  extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    */
    ...
    public static Vector listEmpLevel(String salaryLevel, Vector  posLevelIdx, long oidDivision, long oidDepartment, long payrollG ) {
        return listEmpLevel( salaryLevel, posLevelIdx, oidDivision, oidDepartment, null, PstEmployee.NO_RESIGN,payrollG );
    }
    
     public static Vector listEmpLevel(String salaryLevel, Vector  posLevelIdx,long oidDivision, long oidDepartment, Date minCommencingDate, int status_resign,long payrollG ) {
        DBResultSet dbrs = null;
        Vector result = new Vector(1,1);
        System.out.println("salaryLevel"+salaryLevel);
        if (salaryLevel == "")
         ...
         try {
           ...
           sql = sql + " ORDER BY "+PstPayEmpLevel.fieldNames[PstPayEmpLevel.FLD_START_DATE]+" ASC ";
            ...
            while(rs.next()) {
                ...
                try{employee.setPositionId(rs.getLong("EMP."+PstEmployee.fieldNames[PstEmployee.FLD_POSITION_ID]));}
...
//       
        try {
            String sql = "SELECT plc."+PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_FORMULA] 
                    ...
                    + " WHERE pel." + PstPayEmpLevel.fieldNames[PstPayEmpLevel.FLD_EMPLOYEE_ID] + " in ( "+ employeeId +" ) "
                    + " AND plc." + PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_COMP_CODE] +" = \"GPH\""
