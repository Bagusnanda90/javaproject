...
public class PstSection extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final int FLD_VALID_STATUS = 5;
    public static final int FLD_VALID_START = 6;
    public static final int FLD_VALID_END = 7;
    ...
    public static Section fetchExc(long oid) throws DBException {
        try {
            ...
            section.setValidStatus(pstSection.getInt(FLD_VALID_STATUS));
            section.setValidStart(pstSection.getDate(FLD_VALID_START));
            section.setValidEnd(pstSection.getDate(FLD_VALID_END));
            return section;
    ...
    public static long insertExc(Section section) throws DBException {
        try {
            ...
            pstSection.setInt(FLD_VALID_STATUS, section.getValidStatus());
            pstSection.setDate(FLD_VALID_START, section.getValidStart());
            pstSection.setDate(FLD_VALID_END, section.getValidEnd());
            pstSection.insert();
...
//                        System.out.println("sql section : " + sql);
    ...
    private static void resultToObject(ResultSet rs, Section section) {
        try {
            ...
            section.setValidStatus(rs.getInt(PstSection.fieldNames[PstSection.FLD_VALID_STATUS]));
            section.setValidStart(rs.getDate(PstSection.fieldNames[PstSection.FLD_VALID_START]));
            section.setValidEnd(rs.getDate(PstSection.fieldNames[PstSection.FLD_VALID_END]));
        } catch (Exception e) {
        }
    }
     ...
     */
     ...
     /* Add by Hendra Putu | 2015-12-16 | Larut malam */
     public static Vector listSectionJoinDepart() {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try { 
            String sql = "SELECT "+TBL_HR_SECTION+"."+fieldNames[FLD_SECTION_ID]+", " +TBL_HR_SECTION+"."+fieldNames[FLD_SECTION]+", ";
            sql += PstDepartment.TBL_HR_DEPARTMENT+"."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT]+" FROM " + TBL_HR_SECTION;
            sql += " INNER JOIN "+PstDepartment.TBL_HR_DEPARTMENT+" ON ";
            sql += TBL_HR_SECTION+"."+fieldNames[FLD_DEPARTMENT_ID]+"="+PstDepartment.TBL_HR_DEPARTMENT+"."+PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT_ID];

//                        System.out.println("sql section : " + sql);
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                Vector secDept = new Vector();
                secDept.add(rs.getLong(fieldNames[FLD_SECTION_ID]));
                secDept.add(rs.getString(fieldNames[FLD_SECTION]));
                secDept.add(rs.getString(PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT]));
                lists.add(secDept);
            }
            rs.close();
            return lists;
    ...
    public static Hashtable<String, String> listMapSecName(int limitStart, int recordToGet,  String whereClause, String order) {
        ...
        try {
            ...
            while (rs.next()) {
                Section section = new Section();
                resultToObject(rs, section);
                lists.put(""+section.getOID(),section.getSection());
            }
            rs.close();
            return lists;
