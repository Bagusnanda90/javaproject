...
//            System.out.println("SQL Leave Application : " + sql);
    ...
    public static Vector searchLeaveApplicationList(SrcLeaveApp objSrcLeaveApp, int start, int recordToGet) {
        ...
        try {
            ...
            String strCanceled = "";
            if (objSrcLeaveApp.isCanceled()) {
                strCanceled = " LA." + PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_DOC_STATUS] + " = 4";
            }
            ...
            if (strCanceled != null && strCanceled.length() > 0) {                
                if (sWhere != null && sWhere.length() > 0) {
                    if (status == true) {
                        sWhere = sWhere + " OR " + strCanceled;
                    } else {
                        sWhere = sWhere + " AND (" + strCanceled;
                    }
                } else {
                    sWhere = sWhere + " AND (" + strCanceled;
                }
    ...
    public static float countDP(long AppId, long EmpId) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM(" + PstDpStockTaken.fieldNames[PstDpStockTaken.FLD_TAKEN_QTY] + ") FROM " + PstDpStockTaken.TBL_HR_DP_STOCK_TAKEN + " WHERE "
                    + PstDpStockTaken.fieldNames[PstDpStockTaken.FLD_EMPLOYEE_ID] + " = '" + EmpId + "' AND "
                    + PstDpStockTaken.fieldNames[PstDpStockTaken.FLD_LEAVE_APPLICATION_ID] + " = '" + AppId + "'";
    ...
    public static boolean getBeforeDayExpired( Date dateTkn, Position positionOfUser) {
        ...
        int defaultDeadline = positionOfUser.getDeadlineScheduleLeaveBefore();
        
        String valueName = PstSystemProperty.getValueByName("CLIENT_NAME");
        
        if (valueName.equals("PEPITO")) {
            defaultDeadline = 720;
        }
        
        dateDeadBefore.setHours(dateDeadBefore.getHours() - defaultDeadline);
     ...
     * SAMA
     */
    public static boolean getAfterExpiredDate( AlStockTaken alStockTaken) {
        boolean result = true;
        AlStockManagement alStockManagement = new AlStockManagement();
        try {
        ...
        if (alStockTaken.getTakenDate().after(alStockManagement.getExpiredDate())){
             int LoS = (int) DateCalc.dayDifference(alStockTaken.getTakenDate(), alStockTaken.getTakenFinnishDate());
                    
            if ( LoS > alStockManagement.getEntitled()){
                return false;
            }
                ...
                for (int idxAl = 0; idxAl < listTknAl.size(); idxAl++) {
                    ...
                    for (int idxAlTkn = 0; idxAlTkn < (dayDiff+1); idxAlTkn++) {
                        ...
                        if (schId != 0) {
                            ...
                            int schedule = SessEmpSchedule.updateSchedule(strtDate, schId, oidAl);
                            int status = SessEmpSchedule.updateScheduleStatus(strtDate, schId, 7);
...
//                        
//                        if (schId != 0) {
//                            int statusReason = SessEmpSchedule.updateScheduleReasonStatusAndSchedule(strtDatenew, schId, intAl,"AL");
//                        }
                ...
                for (int idxDp = 0; idxDp < listTknDp.size(); idxDp++) {
                    ...
                    for (int idxDpTkn = 0; idxDpTkn < (dayDiff+1); idxDpTkn++) {
                        ...
                        if (schId != 0) {
                            ...
                            int schedule = SessEmpSchedule.updateSchedule(strtDate, schId, oidDp);
                            int status = SessEmpSchedule.updateScheduleStatus(strtDate, schId, 7);
...
//                    }
                ...
                for (int idxSp = 0; idxSp < listTknSp.size(); idxSp++) {
                    ...
                    int dayDiff = (int) DateCalc.dayDifference(specialUnpaidLeaveTaken.getTakenFinnishDate(), specialUnpaidLeaveTaken.getTakenDate());
