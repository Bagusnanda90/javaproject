...
public class PstEmpRelevantDoc extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
 ...
 public static Vector listDataTable(int limitStart,int recordToGet, String whereClause, String order){
		...
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT EMP.* FROM "+PstEmpRelevantDoc.TBL_HR_EMP_RELEVANT_DOC+" AS EMP "
                                + "INNER JOIN "+PstEmpRelevantDocGroup.TBL_EMP_RELVT_DOC_GRP+" AS REL "
                                + "ON EMP."+PstEmpRelevantDoc.fieldNames[PstEmpRelevantDoc.FLD_EMP_RELVT_DOC_GRP_ID]+" = "
                                + "REL."+PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_EMP_RELVT_DOC_GRP_ID]+""; 
			if(whereClause != null && whereClause.length() > 0)
				sql = sql + " WHERE " + whereClause;
			if(order != null && order.length() > 0)
				sql = sql + " ORDER BY " + order;
 ...
 public static int getCountDataTable(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(EMP."+PstEmpRelevantDoc.fieldNames[PstEmpRelevantDoc.FLD_DOC_RELEVANT_ID]+") "
                    + " FROM "+PstEmpRelevantDoc.TBL_HR_EMP_RELEVANT_DOC+" AS EMP "
                    + "INNER JOIN "+PstEmpRelevantDocGroup.TBL_EMP_RELVT_DOC_GRP+" AS REL "
                    + "ON EMP."+PstEmpRelevantDoc.fieldNames[PstEmpRelevantDoc.FLD_EMP_RELVT_DOC_GRP_ID]+" = "
                    + "REL."+PstEmpRelevantDocGroup.fieldNames[PstEmpRelevantDocGroup.FLD_EMP_RELVT_DOC_GRP_ID]+""; 
            if (whereClause != null && whereClause.length() > 0) {
