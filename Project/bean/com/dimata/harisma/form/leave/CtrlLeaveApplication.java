    ...
    public int action(int cmd, long oidLeaveApplication, Vector vectOfLeaveAppDetail,String approot) {
        ...
        switch (cmd) {
            ...
            case Command.SAVE:
                ...
                /**
                 * @DESC : PENGECEK STATUS SCHEDULE DARI APPROVED KE DRAFT
                  * Jika iLeaveMinuteEnable =1 maka menggunakan konfigurasi jam-jam'an
                 */
                    ...
                    }catch(Exception e){System.out.println("Exeception LEAVE_MINUTE_ENABLE:"+e);}
                 ...
                 * jika iLeaveMinuteEnable = 0 maka mengukan konfigurasi full day
                 */
                 //update by satrya 2012-08-03
                ...
                if (SessEmpSchedule.GetConfigurasiUpdtSch() == PstEmpSchedule.UPDATE_SCHEDULE_AFTER_APPROVED && iLeaveMinuteEnable !=1) {
                    ...
                    /**
                     * @DEC : UNTUK MENGUPDATE STATUS SCHEDULE JIKA STATUS DOCUMENT APPROVED
                     */
                ...
                //if (this.leaveApplication.getDocStatus() < 2){
...
//                }
    ...
    public static void sendEmail(LeaveApplication leaveApp,String approot) {
        ...
        //update by satrya 2013-10-30 
        /**
         * String txtMessage =( (leaveApp.getDocStatus() == PstLeaveApplication.FLD_STATUS_APPlICATION_APPROVED) ?  
...
+"</table>";
    ...
    /**
     * Keterangan: untuk sending email
     * create by satrya 2013-12-10
     * @param leaveApp
     * @param approot
        ...
        try {
            for (int i = 0; i < listRec.size() && i < 3; i++) {
                ...
                if (req.getEmailAddress() != null && req.getEmailAddress().length() > 0 && !req.getEmailAddress().equalsIgnoreCase("-")) {
                    ...
                    boolean tidakada = CheckedNotif(subject, req.getEmailAddress(), leaveApp.getOID());
                    if (tidakada){
                       recx.add(req.getEmailAddress());
                    }        
                }
...
//                        host, port, username, password, SSL,attacment,configEmailWithImage);
                ...
                for (int xx = 0 ; xx < recx.size() ;xx++){
                    String emailAdd = (String) recx.get(xx);
                            try {
                                LogSysHistory logSysHistory = new LogSysHistory();

                                logSysHistory.setLogLoginName(emailAdd);
                                logSysHistory.setLogDocumentType("Leave Approval Notification");
                                logSysHistory.setLogDetail(subject);
                                logSysHistory.setLogDocumentId(leaveApp.getOID());
    ...
    public static Vector getListNextApproval(LeaveApplication objLeaveApplication, Employee objEmployee) {
        ...
        if (  objLeaveApplication.getOID() != 0 && ( 
                /* jika  bukan level manager ke bawah maka setelah approval dep head approval HRD */
                ((maxApproval == I_Leave.LEAVE_APPROVE_1 || maxApproval == I_Leave.LEAVE_APPROVE_2)
                && objLeaveApplication.getDepHeadApproval()!=0 && objLeaveApplication.getHrManApproval()==0) ||
                /* jika level manager ke atas maka  , dephead approval */               
                 ((maxApproval == I_Leave.LEAVE_APPROVE_3 || maxApproval == I_Leave.LEAVE_APPROVE_4 || maxApproval == I_Leave.LEAVE_APPROVE_5)
                && objLeaveApplication.getDepHeadApproval()==0 && objLeaveApplication.getHrManApproval()==0))) {            
        ...
        if (objLeaveApplication.getOID() != 0 &&
                ...
                && objLeaveApplication.getGmApproval()==0 ) {            
                            try {   String emailNotiftoFinalApprover = ""+PstSystemProperty.getValueByName("EMAIL_NOTIFICATION_GM"); //0= off 1=on
