...
public class PstExperience extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_HR_WORK_HISTORY_PAST = "hr_work_history_past";//"HR_WORK_HISTORY_PAST";
    ...
    public static final int FLD_MOVE_REASON = 6;
        //kartika 2015-09-16 
    public static final int FLD_PROVIDER_ID = 7;  // untuk karyawan yg outsource
    public static final int FLD_SALARY_RECEIVED = 8;
    ...
    public static Experience fetchExc(long oid) throws DBException {
        try {
            ...
            experience.setStartDate(pstExperience.getDate(FLD_START_DATE));
            experience.setEndDate(pstExperience.getDate(FLD_END_DATE));
            ...
            experience.setProviderID(pstExperience.getLong(FLD_PROVIDER_ID));
            experience.setSalaryReceived(pstExperience.getString(FLD_SALARY_RECEIVED));
            return experience;
    ...
    public static long insertExc(Experience experience) throws DBException {
        try {
            ...
            pstExperience.setDate(FLD_START_DATE, experience.getStartDate());
            pstExperience.setDate(FLD_END_DATE, experience.getEndDate());
            ...
            pstExperience.setLong(FLD_PROVIDER_ID, experience.getProviderID());
            pstExperience.setString(FLD_SALARY_RECEIVED, experience.getSalaryReceived());
    ...
    private static void resultToObject(ResultSet rs, Experience experience) {
        try {
            ...
            experience.setStartDate(rs.getDate(PstExperience.fieldNames[PstExperience.FLD_START_DATE]));
            experience.setEndDate(rs.getDate(PstExperience.fieldNames[PstExperience.FLD_END_DATE]));
            ...
            experience.setProviderID(rs.getLong(PstExperience.fieldNames[PstExperience.FLD_PROVIDER_ID]));
            experience.setSalaryReceived(rs.getString(PstExperience.fieldNames[PstExperience.FLD_SALARY_RECEIVED]));
        } catch (Exception e) {
        }
    }
    ...
    public static int getCount2(SrcEmployee srcEmployee) {
        ...
        try {
            String sql = "SELECT COUNT(WHP." + PstExperience.fieldNames[PstExperience.FLD_WORK_HISTORY_PAST_ID] + ") FROM " + PstExperience.TBL_HR_WORK_HISTORY_PAST + " WHP INNER JOIN "
                    + PstEmployee.TBL_HR_EMPLOYEE + " EMP ON WHP." + PstExperience.fieldNames[PstExperience.FLD_EMPLOYEE_ID] + "=EMP." + PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]
                    ...
                    + " = HR_LEV." + PstLevel.fieldNames[PstLevel.FLD_LEVEL_ID]) + " WHERE " + sessEmployee.whereList(srcEmployee) + "GROUP BY WHP." + PstExperience.fieldNames[PstExperience.FLD_EMPLOYEE_ID]
                    + " ORDER BY COUNT(WHP." + PstExperience.fieldNames[PstExperience.FLD_WORK_HISTORY_PAST_ID] + ") DESC LIMIT 1";
    ...
    public static Vector listDataTable(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT EMP.* FROM "+PstExperience.TBL_HR_WORK_HISTORY_PAST+""
                    + " AS EMP LEFT JOIN "+PstContactList.TBL_CONTACT_LIST+""
                    + " AS EXP ON EMP."+PstExperience.fieldNames[PstExperience.FLD_PROVIDER_ID]+" ="
                    + " EXP."+PstContactList.fieldNames[PstContactList.FLD_CONTACT_ID]+"";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            if (order != null && order.length() > 0) {
    ...
    public static int getCountDataTable(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(EMP."+PstExperience.fieldNames[PstExperience.FLD_WORK_HISTORY_PAST_ID]+")"
                    + " FROM "+PstExperience.TBL_HR_WORK_HISTORY_PAST+""
                    + " AS EMP LEFT JOIN "+PstContactList.TBL_CONTACT_LIST+""
                    + " AS EXP ON EMP."+PstExperience.fieldNames[PstExperience.FLD_PROVIDER_ID]+" ="
                    + " EXP."+PstContactList.fieldNames[PstContactList.FLD_CONTACT_ID]+"";
            if (whereClause != null && whereClause.length() > 0) {
