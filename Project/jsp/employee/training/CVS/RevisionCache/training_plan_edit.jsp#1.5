...
<script language="JavaScript">	
	...
	function cmdChangeDept(){
        ...
        function cmdAddTrainer() {               
                var myWindow = window.open("<%=approot%>/employee/training/input_trainer_new.jsp", null, "height=550,width=600,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes");
                myWindow.focus();
        }
...
*/
                        ...
                        <%=ControlCombo.draw(FrmTrainingActivityPlan.fieldNames[FrmTrainingActivityPlan.FRM_FIELD_POSITION_ID], null, selected_pos, posValue, posKey, "onChange=\"javascript:cmdChangeDept()\" ", "formElemen")%>
                    </div>
