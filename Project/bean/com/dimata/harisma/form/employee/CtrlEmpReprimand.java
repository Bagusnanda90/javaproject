...
import static com.dimata.harisma.form.employee.CtrlEmpWarning.RSLT_FORM_INCOMPLETE;
import static com.dimata.harisma.form.employee.CtrlEmpWarning.RSLT_OK;
...
public class CtrlEmpReprimand extends Control implements I_Language {
    ...
    public int action(int cmd, long oid, HttpServletRequest request, String loginName, long userId, String oidDeleteAll){
        return action(cmd,oid,request,loginName,userId,oidDeleteAll,"");
    }
    
    public int action(int cmd, long oid, HttpServletRequest request, String loginName, long userId, String oidDeleteAll, String object) {
        msgString = "";
        ...
        switch (cmd) {
            ...
            case Command.SAVE:
                ...
                if (reprimand.getOID() == 0) {    // insert
                    try {
                        ...
                        empDocList.setEmp_doc_id(this.reprimand.getEmpDocId());
                        empDocList.setEmployee_id(this.reprimand.getEmployeeId());
                        empDocList.setEmp_reprimand(result);
                    ...
                    try {
                        ...
                        String whereList = PstEmpDocList.fieldNames[PstEmpDocList.FLD_REPRIMAND_ID]+ " = " + result; 
            ...
            case Command.DELETE:
                ...
                for (String asset : splits) {
                    if (asset != "") {
                        long oidEmpReprimand = Long.parseLong(asset);
                        if (oidEmpReprimand != 0) {
                            try {
                                oid = PstEmpReprimand.deleteExc(oidEmpReprimand);
                                if (oid != 0) {
                                    msgString = FRMMessage.getMessage(FRMMessage.MSG_DELETED);
                                    excCode = RSLT_OK;
                                } else {
