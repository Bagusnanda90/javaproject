...
public class PstPresence extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language { 
	...
	public static final  int FLD_ANALYZED = 4;
        ...
        public static final  int FLD_MANUAL_PRESENCE = 10;
	...
	public static Presence fetchExc(long oid) throws DBException{ 
		try{ 
			...
			presence.setAnalyzed(pstPresence.getInt(FLD_ANALYZED));
                        ...
                        presence.setManualPresence(pstPresence.getInt(FLD_MANUAL_PRESENCE));
	...
	public static long insertExc(Presence presence) throws DBException{ 
		try{ 
			...
			pstPresence.setInt(FLD_ANALYZED, presence.getAnalyzed());
                        ...
                        pstPresence.setInt(FLD_MANUAL_PRESENCE, presence.getManualPresence());
 ...
 */
	...
	public static Date getDatePresence(int limitStart,int recordToGet, String whereClause, String order){
		...
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT PRESENCE.* FROM " + TBL_HR_PRESENCE +" AS PRESENCE "
                                + " INNER JOIN "+PstEmployee.TBL_HR_EMPLOYEE +" AS EMP "
                                + " ON PRESENCE."+fieldNames[FLD_EMPLOYEE_ID] + " = "
...
if(selectDate!=null && oidEmployee!=0){		
         ...
         */
        public static Vector listManualCalculation(int limitStart,int recordToGet,  String order, long oidDepartement, String employeeName, Date fromDate, Date toDate , long oidSection, String payrolNum, String empCat,long oidDivision){
	//public static Vector list(int limitStart,int recordToGet,  String order, long oidDepartement, String employeeName, Date fromDate, Date toDate , long oidSection){
		Vector lists = new Vector(); 
		DBResultSet dbrs = null;
		try {
         ...
         */
                public static int getCountManualCalculation(String order, long oidDepartement, String employeeName, Date fromDate, Date toDate , long oidSection, String payrolNum, String empCat, long oidDivision){
	//public static Vector list(int limitStart,int recordToGet,  String order, long oidDepartement, String employeeName, Date fromDate, Date toDate , long oidSection){
 ...
 */                
 public static int getCountAnalyseStatus(String order, Date fromDate, Date toDate,String whereClause){
		...
		try{
			...
			presence.setAnalyzed(rs.getInt(PstPresence.fieldNames[PstPresence.FLD_ANALYZED]));
			 ...
			 presence.setScheduleType(rs.getInt(PstPresence.fieldNames[PstPresence.FLD_SCHEDULE_TYPE]));
                         ...
                         presence.setManualPresence(rs.getInt(PstPresence.fieldNames[PstPresence.FLD_MANUAL_PRESENCE]));
		}catch(Exception e){ 
	...
	public static int getCount(String whereClause){
		...
		DBResultSet dbrs = null;
		try {
			String sql = "SELECT COUNT(PRESENCE."+ PstPresence.fieldNames[PstPresence.FLD_PRESENCE_ID] + ") "
                                + " FROM " + TBL_HR_PRESENCE+" AS PRESENCE "
                                + " INNER JOIN "+PstEmployee.TBL_HR_EMPLOYEE +" AS EMP "
                                + " ON PRESENCE."+fieldNames[FLD_EMPLOYEE_ID] + " = "
                                + " EMP."+PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]; 
			if(whereClause != null && whereClause.length() > 0)
...
//update by satrya 2012-09-09
     ...
     * @return 
     */
    public static boolean getPresenceManual(long empId, Date SelectedDate) {
        boolean result = false; 
        DBResultSet dbrs = null;
        try 
        {
                String sql = "SELECT " + PstPresence.fieldNames[PstPresence.FLD_PRESENCE_ID] +
                             ...
                             " = " + empId + 
                             " AND " + PstPresence.fieldNames[PstPresence.FLD_PRESENCE_DATETIME] + 
                             " = \"" + com.dimata.util.Formater.formatDate(SelectedDate,"yyyy-MM-dd HH:mm:ss") + "\"" + 
                             " AND " + PstPresence.fieldNames[PstPresence.FLD_MANUAL_PRESENCE] + 
                             " = " + 1; 
                //System.out.println("presenceExist sql : " + sql);
                dbrs = DBHandler.execQueryResult(sql);
                ResultSet rs = dbrs.getResultSet();                        
