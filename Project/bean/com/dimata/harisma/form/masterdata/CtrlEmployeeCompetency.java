...
public class CtrlEmployeeCompetency extends Control implements I_Language {
    ...
    public int action(int cmd, long oidEmployeeCompetency, HttpServletRequest request, String loginName, long userId, String oidDeleteAll) {
        msgString = "";
            ...
            case Command.DELETE:
                ...
                for (String asset : splits) {
                    if (asset != "") {
                        ...
                        if (oidDiv != 0) {
                            try {
                                long oid = PstEmployeeCompetency.deleteExc(oidDiv);
                                if (oid != 0) {
                                    msgString = FRMMessage.getMessage(FRMMessage.MSG_DELETED);
                                    excCode = RSLT_OK;
                                } else {
