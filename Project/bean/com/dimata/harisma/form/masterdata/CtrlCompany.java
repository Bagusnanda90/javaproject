...
public class CtrlCompany extends Control implements I_Language{
    ...
    public int action(int cmd, long oidCompany) {
        return action (cmd, oidCompany, "");
    }

    public int action(int cmd, long oidCompany, String oidDeleteAll) {
        msgString = "";
        ...
        switch (cmd) {
            ...
            case Command.DELETE:
                ...
                for (String company : splits) {
                    if (company != "") {
                        long oidDoc = Long.parseLong(company);
                        if (oidDoc != 0) {
                            try {
                                long oid = PstCompany.deleteExc(oidDoc);
                                if (oid != 0) {
                                    msgString = FRMMessage.getMessage(FRMMessage.MSG_DELETED);
                                    excCode = RSLT_OK;
                                } else {
